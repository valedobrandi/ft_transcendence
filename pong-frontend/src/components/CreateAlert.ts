export function CreateAlert(message: string, type: 'error' | 'success' = 'error'): HTMLElement {
    ...
    if (type === 'error') {
        ...
        wrapper.classList.add("bg-green-100", "border", "border-green-400", "text-green-700", "px-4", "py-3", "rounded", "relative");
    }
    ...
    wrapper.innerHTML = `
      ...
      <button id="close-alert" class="absolute top-0 bottom-0 right-0 px-4 py-3">
        <svg class="fill-current h-6 w-6 text-${type === 'error' ? 'red' : 'green'}-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
          <title>Close</title>
    ...
    const alertBtn = wrapper.querySelector('#close-alert') as HTMLButtonElement; 
