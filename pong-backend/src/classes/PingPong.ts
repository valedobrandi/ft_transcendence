...
import { CONNECT } from "../sockets/handles/CONNECT.js";
...
class PingPong {
	...
	endMatch() {
		...
		const winner = Array.from(this.players.values()).find(player => player.side === getMatchWinner);
		const loser = Array.from(this.players.values()).find(player => player.side !== getMatchWinner);
		...
		gameEvents.emit('matchEnded', {
			...
			winner,
			loser,
			tournamentId: winner ? winner.tournamentId : undefined
		});
